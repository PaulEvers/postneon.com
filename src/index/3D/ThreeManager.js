...
class ThreeManager {
  constructor({ app }) {
    ...
    document
      ...
      .appendChild(this._3d.renderer.domElement);
  ...
  isVideoPlaying = () =>
    this.app.__.focus && this.app.__.focus.media.userData.type === "video";
  resetVideo = () => (this.isVideoPlaying() ? this.app.__.focus.pause() : null);

  pauseMedia = () => {
    const project = this.app.__.focus;
    if (project && project.media.userData.type === "video") project.pause();
  };
  ...
  initProjects = () => {
    ...
    this._3d.projects.findProject = (name) =>
      this._3d.projects.find((p) => p.name === name);
  ...
  fetchScene = async (url) => {
    ...
    let getMediaType = (url) => {
      if (/\.(jpg|jpeg|JPG|png|webp|avif|gif|svg)$/.test(url)) {
        return "image";
      } else {
        return "video";
      }
    ...
    let getRatio = async (url, mediaType) => {
        if (mediaType === "image") {
          const loadedImage = await loadImage(url);
          return loadedImage.width / loadedImage.height;
        } else if (mediaType === "video") {
          const test = await loadVideo(url);

          return test.videoWidth / test.videoHeight;
        }
    }
    ...
    await gallery.fields.projects.forEach(async (project, i) => {
      ...
      //   if (media.type === "image") {
      //     const loadedImage = await loadImage(media.src);
      //     media.ratio = loadedImage.width / loadedImage.height;
      //     return;
      //   } else if (media.type === "video") {
      //     const test = await loadVideo(media.src);

      //     media.ratio = test.videoWidth / test.videoHeight;
  ...
  rotateMenu = (now) => {
    ...
    this.__.menu.lerpTo = this.lerp(
      this.app.__.menu.lerpTo,
      this.__.menu.lerpTo,
      0.125
    );
    if (this.app.__.orientation === "landscape") {
      this._3d.projects.rotation.y +=
        this.__.menu.speed *
          this.__.menu.delta *
          this.app.__.menu.direction *
          0.9 +
        this.__.menu.lerpTo * this.__.menu.delta;
    } else {
      this._3d.projects.rotation.x +=
        this.__.menu.speed *
          this.__.menu.delta *
          this.app.__.menu.direction *
          0.9 +
  ...
  orderByFocused = () => {
    ...
    const index = this.__.projects.findIndex(
      (project) => project.__.name === focusedProject.__.name
    );
